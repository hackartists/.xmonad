...
Config {
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " <icon=haskell_20.xpm/>   <fc=#666666>|</fc> %UnsafeStdinReader% }{ <fc=#c678dd>%mouselocation%</fc> <fc=#b3afc2> %battery% </fc> <fc=#b3afc2> VL%volume% </fc> <fc=#b3afc2> <action=`mimeo https://github.com/notifications`>GH %gh%</action> </fc> <fc=#b3afc2> <action=`alacritty -e htop`>%uname%</action> </fc> <fc=#ecbe7b> <action=`alacritty -e htop`>%cpu%</action> </fc> <fc=#ff6c6b> <action=`alacritty -e htop`>%memory%</action> </fc> <fc=#51afef> <action=`alacritty -e htop`>%disku%</action> </fc> <fc=#c678dd> <action=`alacritty -e sudo pacman -Syu`>%updates%</action> </fc>  <fc=#46d9ff> <action=`emacsclient -n --eval '(cfw:open-org-calendar)'`>%date%</action> </fc><fc=#666666><fn=1>|</fn></fc>"
       }
